...
export default function FormValidation() {
    ...
    const onSubmitForm= (event) =>{ 
        ...
        for(let f of inputFile.current.files) {
            if (f.size> maxSize* 1000) {
                alert(`ขนาดของแต่ละไฟล์ตของไม่เกิน ${maxSize} KB`)
                event.preventDefault()
                return
            }
        } 
    ...
    return(<div className="mt-3 mb-5 mx-auto p-3 rounded" style={{ width:'400px', background:'#cee'}}>
        <form onSubmit={handleSubmit(onSubmitForm)}>
            <div className="form-group mb-3">
                <label htmlFor="cats"className="form-label">หมวดหมู่ *</label>
                <select id="cats"className="form-select form-select-sm">{cats.map(item=>{
                    return<option value={item[0]}>{item[1]}</option>})}</select></div>
                    <div className="form-group mb-3"><label htmlFor="name">ชื่อสินค้า *</label>
                    ...
                    <div className="form-group mb-3">
                        ...
                        <div className="form-group mb-3">
                            <label htmlFor="price">วันที่เพิ่มสินคา *</label>
                            <input type="date"id="dateAdded"name="dateAdded"
                            className="form-control form-control-sm"/></div>
                            <div className="form-group mb-3">
                                <label htmlFor="file"className="form-label">ภาพสินค้า * (ไม่เกิน 3ไฟล์ ขนาดสูงสุด 300 KB/ไฟล์)</label>
                            ...
                            {return(
                                <div className="form-check form-check-inline mb-2">
                                    ...
                                    <label htmlFor={'opt'+i} className="form-check-label">{opt
                                    }</label></div>)})}
                        <div className="text-center mt-4"><button className="btn btn-sm btn-primary px-4">OK</button>
                </div>
            </form>
        </div>
    )
